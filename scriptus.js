const mainus=document.getElementsByClassName('article_container')[0]
let dataus
...
async function getDataUsBus(){
    const res1u=await fetch("https://saurav.tech/NewsAPI/top-headlines/category/business/us.json")
...
function renderUIUsBus(){
    ...
    <div class="article_content">
        ...
        <p><span>Published At:</span> ${formatPublishedAt(dataus[i].publishedAt)}</p>
        <p>${dataus[i].description}</p>
        <a href="${dataus[i].url}" target="_blank">Click to read more...</a>
    </div>
        `
...
async function getDataUsPol(){
    const res2u=await fetch("https://saurav.tech/NewsAPI/top-headlines/category/sports/us.json")
...
function renderUIUsPol(){
    ...
    <div class="article_content">
        ...
        <p><span>Published At:</span> ${formatPublishedAt(dataus[j].publishedAt)}</p>
        <p>${dataus[j].description}</p>
        <a href="${dataus[j].url}" target="_blank">Click to read more...</a>
    </div>
        `
    mainus.appendChild(articleus)
    }
...
async function getDataUsEnt(){
    const res3u=await fetch("https://saurav.tech/NewsAPI/top-headlines/category/entertainment/us.json")
...
function renderUIUsEnt(){
    ...
    <div class="article_content">
        ...
        <p><span>Published At:</span> ${formatPublishedAt(dataus[k].publishedAt)}</p>
        <p>${dataus[k].description}</p>
        <a href="${dataus[k].url}" target="_blank">Click to read more...</a>
    </div>
        `
    mainus.appendChild(articleus)
    }
...
async function getDataUsTech(){
    const res4u=await fetch("https://saurav.tech/NewsAPI/top-headlines/category/technology/us.json")
...
function renderUIUsTech(){
    ...
    <div class="article_content">
        ...
        <p><span>Published At:</span> ${formatPublishedAt(dataus[l].publishedAt)}</p>
        <p>${dataus[l].description}</p>
        <a href="${dataus[l].url}" target="_blank">Click to read more...</a>
    </div>
        `
    mainus.appendChild(articleus)
    }
